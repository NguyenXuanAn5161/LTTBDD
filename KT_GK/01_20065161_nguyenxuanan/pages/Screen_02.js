...
export default function Screen02({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    if (params && params.isEditing) {
      console.log(params.newJob);
      const updatedList = listJobs.map((item) =>
        item.id === params.id ? { ...item, jobName: params.newJob } : item
      );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity
        onPress={() =>
          navigation.push("Screen03", { name: name, isEditing: false })
        }
        style={styles.btnAdd}
      >
